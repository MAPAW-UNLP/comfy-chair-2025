...
export const RevisoresApp = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16">
        <div className="flex flex-col items-center gap-4 bg-white border-2 border-gray-300 rounded-xl p-8 shadow-lg">
          ...
          <p className="text-lg font-semibold text-gray-700">No hay revisores disponibles</p>
  ...
  const handleAsignar = () => {
    if (asignados >= maxAsignados) {
      toast.error("No se pueden asignar más revisores", {
        description: `Máximo de ${maxAsignados} revisores alcanzado`,
        position: "top-center",
      })
      return false
    }
    setAsignados(asignados + 1)
    return true
  }

  const handleEliminar = () => {
    setAsignados(Math.max(asignados - 1, 0))
  }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <h2 className="relative font-semibold text-2xl tracking-tight my-4 text-center">
        ...
        <div className="text-sm mt-1 tracking-tight font-bold">para el artículo {id}</div>
      ...
      >
        ...
        <div className="mt-auto text-center">
          <div
            className="bg-gray-500 text-white rounded-full px-8 py-2 text-sm font-medium inline-block"
